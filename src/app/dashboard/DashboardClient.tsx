...
interface Props {
  ...
  testResults: Record<string, { score: number; level: string }>;
}

export default function DashboardClient({ userName, lessons, tests, tips, progress, testResults }: Props) {
  const [tipIndex] = useState(() => tips.length ? Math.floor(Math.random() * tips.length) : 0);

  const completedSet = new Set(progress.filter(p => p.completed).map(p => p.lesson_id));
  const totalCompleted = completedSet.size;
  const progressPercent = lessons.length ? Math.round((totalCompleted / lessons.length) * 100) : 0;
  const completedTestsCount = Object.keys(testResults).length;
  const canGetCertificate = (lessons.length > 0 && totalCompleted >= lessons.length) || completedTestsCount >= 3;
